...
export default function QcPage() {
  ...
  const SliderField = ({ label, field, unit="%" }: {label:string, field:string, unit?:string}) => (
    <div className="form-group">
      <label>{label} — <span className="text-emerald-400">{(form as any)[field]}{unit}</span></label>
      <input type="range" min={0} max={50} step={0.5} value={(form as any)[field]}
        onChange={e => setForm({...form, [field]: parseFloat(e.target.value)})}
        style={{ accentColor: "#10b981" }} className="w-full" />
    </div>
  );
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-400 text-sm">Inspeksi material & grading OCC</p>
        </div>
      ...
      {stats.length > 0 && (
        <div className="grid grid-cols-3 gap-4 mb-6">
          {stats.map((s: any) => (
            <div key={s.grade} className="stat-card text-center">
              <div className={`text-3xl font-bold mb-1 ${s.grade === "A" ? "text-emerald-400" : s.grade === "B" ? "text-yellow-400" : "text-red-400"}`}>{s._count}</div>
      ...
      {showForm && (
        <div className="glass rounded-2xl p-6 mb-6 animate-fade-in">
          <h3 className="font-bold mb-4">Form Inspeksi QC</h3>
          ...
          <table>
            ...
            <tbody>
              {inspections.map((q:any) => (
                <tr key={q.id}>
                  <td className="font-mono text-xs text-emerald-400">{q.ticket?.ticketNumber}</td>
                  ...
                  <td className="text-xs text-slate-500">{new Date(q.inspectedAt).toLocaleDateString("id-ID")}</td>
                </tr>
              ))}
            </tbody>
          </table>
