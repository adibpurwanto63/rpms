    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      {!loading && stats.length > 0 && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
          {[
            { label: "Grade A (Diterima)", value: getStat("A"), icon: "✅", variant: "mint" },
            { label: "Grade B (Sortir)", value: getStat("B"), icon: "⚠️", variant: "dark" },
            { label: "Ditolak (Reject)", value: getStat("REJECT"), icon: "🚫", variant: "pink" },
          ].map((k, i) => (
